function retforces = CalcForces(basedroplet, ndrops, mtens,btens, Cspec,gamma,perturb)

    retforces = zeros(ndrops,3);    %vector, 3 dimensions
    for idrop = 1:ndrops
        %Set flags.
        ecalcs = zeros(6,1);
        for c = 1:6
            ...
            if c == 0
                %do nothing
            elseif c == 1
                droplet(idrop).x = basedroplet(idrop).x + perturb;
            elseif c == 2
                droplet(idrop).x = basedroplet(idrop).x - perturb;
            elseif c == 3
                droplet(idrop).y = basedroplet(idrop).y + perturb;
            elseif c == 4
                droplet(idrop).y = basedroplet(idrop).y - perturb;
            elseif c == 5
                droplet(idrop).z = basedroplet(idrop).z + perturb;
            elseif c == 6
                droplet(idrop).z = basedroplet(idrop).z - perturb;
            end
            bRun = 1;
                        ...
                        for j = 1:ndrops
                            if i ~= j
                                ...
                                if droplet(i).barea(j) ~= 0
                                    ...
                                    Cs = Cspec*(1+gamma*dV^2);  %Modify the specific capacitance as needed
                                    %%%%%%%%%%%%%%%%%%%%%%%%%%%
            ...
            ecalcs(c) = droplet(idrop).energy;
        end
        xforce = -(ecalcs(1)-ecalcs(2))/(2*perturb);
        yforce = -(ecalcs(3)-ecalcs(4))/(2*perturb);
        zforce = -(ecalcs(5)-ecalcs(6))/(2*perturb);
        retforces(idrop,:) = retforces(idrop,:)+ [xforce yforce zforce];
    end
end